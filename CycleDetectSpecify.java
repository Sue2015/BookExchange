...
public class CycleDetectSpecify {
	static Tarjan2 t = new Tarjan2();
	...
	public static void main(String[] args) throws Exception {
		...
		try { 
			...
			int book_give = Integer.valueOf(args[2]);
			...
			for(int i=1; i<=size; i++){
				stmt = conn.createStatement();
				if(i==user_want){
					rs = stmt.executeQuery("select book_id from want_book where user_id = '"+i+"' AND book_id = '"+book_want+"'");
				}else{
				...
				while(rs.next()){
					...
					while(temp_rs.next()){
						int tmp_usr = temp_rs.getInt(1);
						if(tmp_usr == user_want && temp_book_id != book_give)
							continue;
						g.get(i-1).add(tmp_usr-1);
					}
					if(book_give == temp_book_id)
						g.get(i-1).add(user_want-1);
				}
			}
			...
			rs = stmt.executeQuery("select user_id from own_book where book_id = '"+book_want+"' AND user_id <> "+user_want);
			...
			else
			{
				...
				for(int i=2; i<temp_result.length-1; i++){
					result += "->";
					int prev = Integer.valueOf(temp_result[i-1]);
					int cur = Integer.valueOf(temp_result[i]);
					String query = "select own_book.book_id from own_book join want_book on own_book.book_id=want_book.book_id where want_book.user_id = '"+prev+"' and own_book.user_id='"+cur+"'";
					//System.out.println(query);
					stmt = conn.createStatement();
				...
				result = result +"->"+book_give+","+user_want;
